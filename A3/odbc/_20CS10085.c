...
int db_fetch()
{
  ...
  int i;
  // Loop to take user input and execute queries
  while(1){
    printf("Enter the query number(Enter -1 to exit): ");
    scanf("%d", &i);
    if(i==1)
    {
      ...
      // Print the column names, if data found
      if(sql_return_val==SQL_SUCCESS||sql_return_val==SQL_SUCCESS_WITH_INFO)
        printf("\nPhysician Name\n");
  ...
  "WHERE Code = (SELECT Medication "
    ...
    {
      char input[1000];
      // Take the procedure name to search for
      printf("\nEnter a Procedure: ");
      fflush(stdin);
      ...
      // Fetch the data
      sql_return_val=SQLFetch(sql_hstmt);
      // Print the column name, if data is found
      if(sql_return_val==SQL_SUCCESS||sql_return_val==SQL_SUCCESS_WITH_INFO)
      printf("\nPhysician Name\n");
      // Print if no data is found
      else if(SQL_NO_DATA==sql_return_val) printf("No Data Found related to the query\n");
      ...
      // If the query number is invalid
      if(i!=-1)
